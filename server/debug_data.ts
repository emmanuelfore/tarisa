...
async function debugData() {
    ...
    console.log("\n--- Issue Calls by Citizen ---");
    for (const c of allCitizens) {
        const [res] = await db.select({ count: count() }).from(issues).where(eq(issues.citizenId, c.id));
        console.log(`Citizen ${c.name} (ID: ${c.id}) has ${res.count} issues`);
    }
